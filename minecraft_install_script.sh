...
STEAMTINKER_CMD="steamtinkerlaunch"
...
setup_steamtinkerlaunch() {
  if command -v "$STEAMTINKER_CMD" &> /dev/null; then
    return 0
  fi

  local STEAMTINKER_PATH="/tmp/steamtinkerlaunch"
  if [[ -x "$STEAMTINKER_PATH" ]]; then
    STEAMTINKER_CMD="$STEAMTINKER_PATH"
    return 0
  fi

  zen_nospam --info --text="Downloading steamtinkerlaunch to /tmp..."
  if ! curl -L -o "$STEAMTINKER_PATH" "https://github.com/sonic2kk/steamtinkerlaunch/raw/refs/heads/master/steamtinkerlaunch"; then
    zen_nospam --error --text="Failed to download steamtinkerlaunch."
    exit 1
  fi
  chmod +x "$STEAMTINKER_PATH"
  STEAMTINKER_CMD="$STEAMTINKER_PATH"
}
...
main() {
  ...
  if [[ "$install_option" == "add_to_steam" ]]; then
    ...
    "$STEAMTINKER_CMD" addnonsteamgame -ep="$main_exe" -an="Minecraft Bedrock (GDK)" -clo="$LAUNCH_OPTIONS"
